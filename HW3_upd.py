...
st.title("HW-03 --> Ankit Waikar")
st.markdown('''**Enter URL**, :rainbow[Select LLM], :green[Select Conversation Behavior], :blue-background[Start Chatting with Bot!] :wave: :thought_balloon:''')
...
if selected_llm_for_chatbot == "OpenAI":
    model_to_use_for_chatbot = "gpt-4" if use_advanced_model else "gpt-4o-mini"
elif selected_llm_for_chatbot == "Claude":
    model_to_use_for_chatbot = "claude-3-opus-20240229" if use_advanced_model else "claude-3-haiku-20240307"
else:  # Gemini
    model_to_use_for_chatbot = "gemini-1.0-pro" if use_advanced_model else "gemini-1.5-flash"
...
def generate_summary(text, instruction, model_to_use):
    if model_to_use in ["gpt-4o-mini", "gpt-4"]:
